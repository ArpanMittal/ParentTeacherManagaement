...
@RequiresApi(api = 23)
public class CustomAdapter extends BaseAdapter implements Filterable, ThemedSpinnerAdapter {
    ...
    private class SimpleFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence prefix) {
            ...
            if (prefix == null || prefix.length() == 0) {
                ArrayList<Map<String, ?>> list = CustomAdapter.this.mUnfilteredData;
                results.values = list;
                results.count = list.size();
            } else {
                String prefixString = prefix.toString().toLowerCase();
                ArrayList<Map<String, ?>> unfilteredValues = CustomAdapter.this.mUnfilteredData;
                int count = unfilteredValues.size();
                ArrayList<Map<String, ?>> newValues = new ArrayList(count);
                for (int i = 0; i < count; i++) {
                    Map<String, ?> h = (Map) unfilteredValues.get(i);
                    if (h != null) {
                        int len = CustomAdapter.this.mTo.length;
                        for (int j = 0; j < len; j++) {
                            for (String word : ((String) h.get(CustomAdapter.this.mFrom[j])).split(" ")) {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            CustomAdapter.this.mData = (List) results.values;
            if (results.count > 0) {
    ...
    public CustomAdapter(Context context, List<? extends Map<String, ?>> data, @LayoutRes int resource, String[] from, @IdRes int[] to) {
        this.mData = data;
    ...
    private void bindView(int position, View view) {
        ...
        if (dataSet != null) {
            android.widget.SimpleAdapter.ViewBinder binder = this.mViewBinder;
                ...
                if (v != null) {
                    ...
                    if (bound) {
                        ...
                        if (data instanceof Boolean) {
                            ...
                            Object obj;
                            StringBuilder append = new StringBuilder().append(v.getClass().getName()).append(" should be bound to a Boolean, not a ");
                            if (data == null) {
                                obj = "<unknown type>";
                            } else {
                                obj = data.getClass();
                            }
                            throw new IllegalStateException(append.append(obj).toString());
                        }
                    } else if (v instanceof TextView) {
                        setViewText((TextView) v, text);
    ...
    public void setViewImage(ImageView v, String value) {
        String url = value;
        try {
            v.setImageResource(R.drawable.loading_thumbnail);
            Picasso.with(this.context1).load("https://img.youtube.com/vi/" + url + "/0.jpg").placeholder((int) R.drawable.loading_thumbnail).error((int) R.drawable.no_thumbnail).resize(480, 360).rotate(0.0f).into(v);
        } catch (NumberFormatException e) {
            v.setImageURI(Uri.parse(String.valueOf(R.drawable.logo)));
        }
    }
